    ...
    $.fn.inlineEditable = function (options, ext) {
        ...
        function makeEditable() {
            ...
            if (!binded) {        // bind event handler only once
                ...
                self.unbind('readable.inlineEditable').bind('readable.inlineEditable', function (e, disable_handlers, cancel) {
                    if (mode != 'read') {
                        var input_id = this.id + '-input';
                        var input = $('#' + input_id);
                        makeReadable.call(input, input.val(), disable_handlers, cancel);
                    }
                });
                binded = true;
            }
        ...
        function makeReadable(new_text, disable_handlers, cancel) {
        ...
        function getText(item) {
            ...
            if (settings.html) {
                if (settings.nl2br) {
                    val = item.html().replace(/<br>/g, '\n');
                } else {
                    val = item.html();
                }
            } else {
                if (settings.nl2br) {
                    val = $.wa.decodeHTML(item.html().replace(/<br>/g, '\n'));
                } else {
                    val = item.text();
                }
            }
        ...
        function setText(item, text) {
            if (settings.html) {
                ...
                if (settings.nl2br) {
                    return item.html($.wa.encodeHTML(text).replace(/\n/g, "<br>"));
                } else {
