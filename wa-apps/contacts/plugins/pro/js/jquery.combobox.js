    ...
    $.widget("custom.combobox", {
        ...
        _createAutocomplete: function () {
            ...
            this.input.bind("autocompleteselect",function (event, ui) {
                ...
                if (ui.item.option && !$(ui.item.option).hasClass('value')) {
                    ...
                    if (opt.data('period')) {
                        var d = self._showPeriodDialog();
                        d.bind('select', function (event, start_datetime, end_datetime) {
                            self._updatePeriodInput(start_datetime, end_datetime);
                            self._removeIcon();
                        });
                        d.bind('cancel', function() {
                            self.input.val('');
            ...
            this.input.bind("autocompleteopen", function (event, ui) {
                ...
                self.ul.find('li.ui-menu-item').each(function () {
                    var el = $(this);
                    var text_el = el.find('.text').width(Math.max(width - el.find('.count').outerWidth(true) - 30, 0));
                    if (text_el.data('sep')) {
                        el.css({
                ...
                this.input.bind("autocompleteopen", function (event, ui) {
                    ...
                    self.ul.find('li.ui-menu-item').each(function () {
                        ...
                        if (el.find('.sep').length) {
                            ...
                            el.find('.text').width(Math.max(width - el.find('.count').outerWidth(true) - 30, 0));
                        }
                    });
        ...
        _createShowAllButton: function () {
            ...
            if (element.find('option').length) {
                this.arrow = $("<a href='javascript:void(0);'><i class='icon16 darr' style='margin-top:1px;'></i></a>")
                    .attr("tabIndex", -1)
                    .appendTo(this.wrapper)
        ...
        _formItem: function (item) {
            var label = item.label;
            if (!_other_version_) {  // this.ns !== 'contact_info.email'
                label = $.wa.encodeHTML(item.name);
            }
            var label = '<span class="text" data-sep="' + (item.sep ? 1 : 0) + '" style="float: left; text-overflow: ellipsis; white-space: nowrap; height: 20px; overflow: hidden; display: block; padding: 0;">' + label;
            if (item.icon) {
