...
class contactsSearchHelper
{
    ...
    public static function getConfig($options = false)
    {
        if (!self::$config) {
            ...
            $event_result = wa('contacts')->event('search.config');
                ...
                foreach ($section['items'] as $item_id => &$item) {
                    if (!empty($item['items'])) {
                        ...
                        if (!empty($item['field_id']) && !empty($options['unwrap_contact_field'])) {
                            self::includeField($item);
                            if ($item['field'] && $item['field'] instanceof waContactCompositeField) {
                                foreach ($item['items'] as $itm_id => &$itm) {
    ...
    private static function getRawConfig($with_apps = true)
    {
        ...
        foreach ($apps as $app_id => $app) {
            if (wa()->appExists($app_id)) {
                $files = array(
                    wa('contacts')->getConfig()->getConfigPath("search/search_{$app_id}.php"),
                    wa('contacts')->getConfig()->getPluginPath('pro')."/lib/config/search/search_{$app_id}.php"
                );
                foreach ($files as $file) {
                    if (file_exists($file)) {
    ...
    public static function updateItem($id, $item_config)
    {
        ...
        waUtils::varExportToFile($config, wa('contacts')->getConfig()->getConfigPath('search/search.php'), true);
...
//                }
    ...
    public static function addWhereForPeriodItem($cond, $item, waContactsCollection $collection, $replace = array())
    {
        ...
        $itm = array('where' => $where);
        return contactsSearchHelper::addWhere($itm, $collection, $replace);
    }
    ...
    public static function getConditionItem($id, $conds = array())
    {
        if (!is_array($conds)) {
            $conds = array();
        }
        $config = contactsSearchHelper::getConfig(array(
            'unwrap_contact_field' => true
        ));
        $keys = array();
        foreach (explode('.', $id) as $k) {
        ...
        if (isset($p['items'])) {
            foreach ($p['items'] as $item_id => &$item) {
                if (isset($item['items']) && !is_array($item['items'])) {
                    ...
                    ) + self::makeItemsListFromQuery($item['items']);
                }
            }
            unset($item);
        }
        return array(
            'item' => $p,
            'count' => self::countOfConditionItem($p, $conds)
        );
    }
    ...
    private static function getNameOfValue($item, $value, $conds = null)
    {
        ...
        if (isset($item['field']) && is_object($item['field']) && method_exists($item['field'], 'getOptions')) {
            ...
            if (is_string($item['items'])) {
                $itms = contactsSearchHelper::makeItemsListFromQuery($item['items']);
    ...
    {
        $item_ids = wa()->getUser()->getSettings('contacts', 'search_form_items');
        if ($item_ids) {
    ...
    public static function prepare(waContactsCollection $collection, $hash, $auto_title)
    {
                    ...
                    if (isset($config['contact_info']['items'][$item_id]['field_id'])) {
                        if (isset($val_item['val'])) {
                            if ($config['contact_info']['items'][$item_id]['field_id'] === 'phone') {
                                $pure_val = preg_replace("/[^\d]+/", "", $val_item['val']);
        ...
        foreach ($conds as $section_id => $section) {
            ...
            foreach ($section as $item_id => $conds_ar) {
                ...
                if (!contactsSearchHelper::isNumericArray($conds_ar)) {
                    $conds_ar = array($conds_ar);
                }
                            ...
                            if (isset($cond[$it_id]) || ($it_id === ':period' && isset($cond['period']))) {
                                if (!empty($item['field'])) {
                                    ...
                                    if ($storage && $storage instanceof waContactStorage)
                                    {
                                        ...
                                        if (!contactsSearchHelper::addJoin($item, $collection, $section_join_aliases)) {
                                            contactsSearchHelper::addLeftJoin($item, $collection, $section_join_aliases);
                                        }
                                        if ($it_id === ':period') {
                                            contactsSearchHelper::addWhereForPeriodItem($cond['period'], $it, $collection, $section_join_aliases);
                                        } else {
                                            contactsSearchHelper::addWhere($it, $collection, $section_join_aliases);
                                            contactsSearchHelper::addHaving($it, $collection, $section_join_aliases);
                                        }
                                    }
                                } else {
                                    if (isset($cond['val'])) {
                                        $al = contactsSearchHelper::addJoin($it, $collection,
                                        ...
                                        if ($it_id === ':period') {
                                            contactsSearchHelper::addWhereForPeriodItem($cond, $it, $collection, $replace);
                                        } else {
                                            contactsSearchHelper::addWhere($it, $collection, $replace);
                                            contactsSearchHelper::addHaving($it, $collection, $replace);
                                        }
                                    } else {
                                        foreach ($cond as $val_item_id => $val_item) {
                                            if ($val_item_id === $it_id || ($it_id{0} === ':' && substr($it_id, 1) === $val_item_id)) {
                                                if (isset($val_item['val'])) {
                                                    ...
                                                    $al = contactsSearchHelper::addJoin($it, $collection, $replace);
                                                    if (!$al) {
                                                        $al = contactsSearchHelper::addClassJoin($it, $collection, $replace, array(
                                                            'conds' => $conds
                                                        ));
                                                    }
                                                    $replace = $section_join_aliases + $item_join_aliases + array(
                                                    ...
                                                    if ($it_id === ':period' && $val_item_id === 'period') {
                                                        contactsSearchHelper::addWhereForPeriodItem($val_item, $it, $collection, $replace);
                                                    } else {
                                                        contactsSearchHelper::addExtWhere($it, $collection, array(
                                                            'replace' => $replace,
                                                            'val_item' => $val_item
                                                        ));
                                                        contactsSearchHelper::addExtHaving($it, $collection, array(
                                                            'replace' => $replace,
                                                            'val_item' => $val_item
                                                        ));
                                                    }
                                                    contactsSearchHelper::addClassWhere($it, $collection, $val_item, $replace, array(
                                                        'conds' => $conds
                                                    ));
                                                    contactsSearchHelper::addClassHaving($it, $collection, $val_item, $replace, array(
                                                        'conds' => $conds
                                                    ));
                                                } else {
                                                    foreach ($val_item as $vl_item_id => $vl_item) {
                                                        if (isset($vl_item['val']) && !isset($it['items'][$vl_item_id]) && !isset($it['items'][":{$vl_item_id}"]))        // no such cases: blank, not_blank or :period etc.
                                                        {
                                                            ...
                                                            $al = contactsSearchHelper::addJoin($it, $collection, $replace);
                                                            if (!$al) {
                                                                contactsSearchHelper::addClassJoin($it, $collection, $replace, array(
                                                                    'conds' => $conds
                                                                ));
                                                            }
                                                            ...
                                                            contactsSearchHelper::addClassWhere($it, $collection, array($vl_item_id => $vl_item), $replace, array(
                                                                'conds' => $conds
                                                            ));
                                                            contactsSearchHelper::addClassHaving($it, $collection, array($vl_item_id => $vl_item), $replace, array(
                                                                'conds' => $conds
                                                            ));
                                                        }
                                                    }
                                ...
                                if (isset($it['items']) && is_array($it['items'])) {
                                    if (isset($cond[$it_id]['val'])) {
                                        ...
                                        if ($i) {
                                            contactsSearchHelper::addWhere($i, $collection);
                                            contactsSearchHelper::addHaving($i, $collection);
                                        } else {
                                            if (isset($it['items'][':values'])) {
                                                ...
                                                $al = contactsSearchHelper::addJoin($it['items'][':values'], $collection, $replace);
                                                if (!$al) {
                                                    $al = contactsSearchHelper::addClassJoin($it, $collection, $replace, array(
                                                        'conds' => $conds
                                                    ));
                                                ...
                                                contactsSearchHelper::addExtWhere($it['items'][':values'], $collection, array(
                                                    'replace' => $replace,
                                                    'val_item' => $cond[$it_id]
                                                ));
                                                contactsSearchHelper::addExtHaving($it['items'][':values'], $collection, array(
                                                    'replace' => $replace,
                                                    'val_item' => $cond[$it_id]
                                                ));
                                                contactsSearchHelper::addClassWhere($it, $collection, $cond[$it_id], $replace, array(
                                                    'conds' => $conds
                                                ));
                                                contactsSearchHelper::addClassHaving($it, $collection, $cond[$it_id], $replace, array(
                                                    'conds' => $conds
                                                ));
                                            }
                                            if (isset($it['items'][':period'])) {
                                                contactsSearchHelper::addWhereForPeriodItem($cond[$it_id], $it['items'][':period'], $collection, $replace);
                                            }
                                        }
                                    } else {
                                        foreach ($it['items'] as $i_id => $i) {
                                            if (isset($cond[$it_id][$i_id]) || ($i_id === ':period' && isset($cond[$it_id]['period']))) {
                                                if ($i_id === ':period') {
                                                    $al = contactsSearchHelper::addJoin($i, $collection, $section_join_aliases + $item_join_aliases);
                                                    if (!$al) {
                                                        $al = contactsSearchHelper::addLeftJoin($i, $collection, $section_join_aliases + $item_join_aliases);
                                                    }
                                                    contactsSearchHelper::addWhereForPeriodItem($cond[$it_id]['period'], $i, $collection,
                                                            $section_join_aliases + $item_join_aliases + array(
                                                                ':table' => $al
                                                            ));
                                                } else {
                                                    ...
                                                    contactsSearchHelper::addWhere($i, $collection, $section_join_aliases + $item_join_aliases + array(
                                                        ':table' => $al
                                                    ));
                                                    contactsSearchHelper::addHaving($i, $collection, $section_join_aliases + $item_join_aliases + array(
                                                        ':table' => $al
                                                    ));
                                                }
                                            }
