...
return array(
    'contact_info' => array(
        'name' => 'Contact info',       // _wp('Contact info')
        'items' => array(
            'name' => array(
                'name' => 'Name',      // _wp('Name')
                'title' => false,
                ...
                'items' => array(
                    ...
                    'title' => array(
                        ...
                        'items' => array(
                            'blank' => array(
                                'name' => 'Empty',     // _wp('Empty')
                                'where' => "c.title = ''"
                            ),
                            'not_blank' => array(
                                'name' => 'Not empty',     // _wp('Not empty')
                                'where' => "c.title != ''"
                            ),
                            ':sep' => array(),
                            ':values' => array(
            ...
            'contact_type' => array(
                'name' => 'Contact type',  // _wp('Contact type')
                'readonly' => true,
                'items' => array(
                    'person' => array(
                        'name' => 'Person',        // _wp('Person')
                        'where' => 'c.is_company = 0',
                    ),
                    'company' => array(
                        'name' => 'Company',   // _wp('Company')
            ...
            'birthday' => array(
                ...
                'items' => array(
                    ...
                    'month' => array(
                        'name' => 'in the nearest month',      // _wp('in the nearest month')
                        'where' => "c.birth_day IS NOT NULL AND c.birth_month IS NOT NULL AND
                    STR_TO_DATE(CONCAT(YEAR(NOW()), '-', c.birth_month, '-', c.birth_day), '%Y-%m-%d') >= NOW() AND (
            ...
            'creating' => array(
                'name' => 'Creating method and date',       // _wp('Creating method and date')
                'multi' => true,
                'items' => array(
                    'method' => array(
                        'name' => 'Method',         // _wp('Method')
                        'readonly' => true,
                        'items' => array(
                            ':values' => array(
    ...
    'activity' => array(
        'name' => 'Activity',  // _wp('Activity')
        'items' => array(
            'action_by' => array(
                'name' => 'Performed an action',        // _wp('Performed an action')
                'multi' => true,
                'join' => array(
                    'table'=> 'wa_log'
                ),
                'items' => array(
                    'action' => array(
                        'name' => 'Action',                     // _wp('Action')
                        'readonly' => true,
                        'skip_first_space' => true,
                        'items' => array(
                            'any_action' => array(
                                'name' => 'any action',         // _wp('any action')
                            ),
                            ':sep' => array(),
                            ':values' => array(
            ...
            'action_to' => array(
                'name' => 'Applied an action to',       // _wp('Applied an action to')
                'multi' => true,
                'join' => array(
                    'table' => 'wa_log',
            ...
            'event_participants' => array(
                'name' => 'Event participants',     // _wp('Event participants')
                'multi' => true,
                'joins' => array(
            ...
            'access' => array(
                'name' => 'Access',         // _wp('Access')
                'readonly' => true,
                'items' => array(
                    'forbidden' => array(
                        'name' => 'Forbidden',       // _wp('Forbidden')
                        'where' => 'c.is_user=-1'
                    ),
                    'customer_portal' => array(
                        'name' => 'Customer portal only',   // _wp('Customer portal only'),
                        'where' => 'c.is_user=0'
                    ),
                    'backend' => array(
                        'name' => 'Backend',
