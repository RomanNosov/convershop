...
class mailerMessageModel extends waModel
{
    ...
    public function getByRecipient($recipient_id, $message_statuses = null)
    {
        $where = array();
        $where[] = "m.is_template = 0";
        if ($message_statuses !== null) {
            $where[] = "m.status IN (" . implode(",", array_map('intval', (array) $message_statuses)) . ")";
        }
        $recipient_id = array_map('intval', (array) $recipient_id);
        $where[] = "ml.contact_id IN(" . implode(",", $recipient_id) . ")";

        $fields = array(
            'm.*'
        );
        $mml = new mailerMessageLogModel();
        foreach (array_keys($mml->describe()) as $field) {
            $fields[] = "ml." . $field . " AS mml_" . $field;
        }

        $sql = "SELECT " . implode(",", $fields) . "
            FROM `mailer_message` AS m
            JOIN `mailer_message_log` AS ml ON ml.message_id = m.id
            WHERE " . implode(" AND ", $where) . "
            ORDER BY finished_datetime DESC";
        return $this->query($sql)->fetchAll('id');
    }

    public function countByRecipient($recipient_id, $message_statuses = null, $group_my_log_statuses = false)
    {
        $where = array();
        $where[] = "m.is_template = 0";
        if ($message_statuses !== null) {
        ...
        if ($group_my_log_statuses) {
            $sql .= " GROUP BY ml.status";
        }

        return !$group_my_log_statuses ? $this->query($sql)->fetchField() : $this->query($sql)->fetchAll('status', true);
    }
