...
class mailerSubscriberModel extends waModel
{
    ...
    public function getByContact($contact_id, $with_email = false)
    {
        $join = array();
        $join[] = "JOIN {$this->table} s ON l.id = s.list_id";
        if ($with_email) {
            $join[] = "JOIN wa_contact_emails e ON s.contact_email_id = e.id";
        }
        $sql = "SELECT *
    ...
    protected function getContact($subscriber)
    {
        ...
        $contact_id = null;
        $contact_id = $cem->getContactIdByNameEmail(ifset($subscriber['name'], ''), ifset($subscriber['email'], ''), false);
