...
class shopFrontendAxiomusCheckoutController extends waJsonController
{
	...
	if (waRequest::get('type') == "updateAllStatus") {
			...
			$orders = $model->query("SELECT p.*
                ...
                WHERE p.name = \"axiomus_okay\" 
				  AND (SELECT count(*) 
					   FROM shop_order_params pp 
					   WHERE pp.order_id = p.order_id 
					     AND pp.name = \"axiomus_updated\") = 0");
            $orders = $orders->fetchAll();

            foreach ($orders as $order) {
                $model->exec("INSERT INTO shop_order_params (order_id, name, value) 
				              VALUES (".$order["order_id"].", \"axiomus_updated\", \"".(time() - 650)."\")");
            }
			
		$orders = $model->query(
			   "SELECT o.id, o.state_id, p.value AS axiomus_okay, p2.value AS axiomus_updated 
                FROM shop_order o 
                LEFT JOIN shop_order_params p ON p.order_id = o.id 
                LEFT JOIN shop_order_params p2 ON p2.order_id = o.id 
                WHERE p.name = \"axiomus_okay\" 
