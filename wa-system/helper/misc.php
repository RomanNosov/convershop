...
function tg_sizes() {
    $model = new waModel();
    $sizes = $model->query("SELECT `id` FROM `shop_feature` WHERE code = 'size'")->fetchAll();
    $sizes = $model->query("SELECT * FROM `shop_feature_values_varchar` WHERE feature_id = ".$sizes[0]["id"]." ORDER BY sort")->fetchAll();
    
    foreach ($sizes as $size) {
        $id = $size["id"];
        $size = $size["value"];
        $checked = in_array($id, $_GET["size"]) ? " checked" : "";
        echo "<li><input id=\"s$id\" type=\"checkbox\" name=\"size[]\" value=\"$id\"$checked><label for=\"s$id\">$size</label></li>";
    }
}

function tg_bought_today() {
    $model = new waModel();

    return $model->query("SELECT p.product_id AS id
        FROM `shop_order` o
        INNER JOIN `shop_order_items` p ON p.order_id = o.id  
        WHERE 1
...
function tg_location() {
    $user_ip = getenv('REMOTE_ADDR');
    $geo = unserialize(file_get_contents("http://www.geoplugin.net/php.gp?ip=$user_ip"));

    if (!$geo["geoplugin_city"]) return "";

    $url = 'https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20140808T124547Z.d3ed55173a9d09b4.3c1807c3bbe26830ba0c1d4dc633a5e3afa0b9b7&lang=en-ru&text='.$geo["geoplugin_city"];
    $arr = file_get_contents($url);

    $arr = json_decode($arr, true);
    $arr = $arr["text"][0];

    return "Вы из города <a href=\"#\">$arr</a>";
}
